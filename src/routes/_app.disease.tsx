...
import { LEAF_DISEASE } from "@/lib/photography";
  ...
  head: () => ({
    meta: [
      { title: "Leaf Scan — PREDI-FARM X" },
      {
        name: "description",
        content:
          "Upload a leaf or plant photo and get an AI diagnosis with confidence, look-alikes and a safe treatment window.",
      },
...
function DiseasePage() {
  ...
  const state = block ? cropStateFor(block.id) : null;
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          ...
          <div className="space-y-4">
            {block && (
              <Panel className="p-5">
                <SectionHeading
                  title="What the model already knows"
                  hint="Before you photograph anything"
                  icon={ShieldAlert}
                />
                <p className="text-sm leading-relaxed text-muted-foreground">
                  Modelled pressure on {block.name} is{" "}
                  <span className="font-semibold text-foreground">{block.diseaseRisk}%</span>,
                  driven by humidity and canopy stage
                  {state ? ` (${state.stage.name.toLowerCase()})` : ""}. A photo either confirms or
                  overrides this.
                </p>
                <Meter
                  className="mt-4"
                  value={block.diseaseRisk}
                  tone={block.diseaseRisk >= 55 ? "bad" : "warn"}
                />
                ...
                {state?.withinPhi && (
                  <p className="mt-4 text-sm leading-relaxed text-destructive">
                    Do not spray whatever the diagnosis says. Harvest is in{" "}
                    {state.daysToHarvest} days and the pre-harvest interval for this crop is{" "}
                    {state.crop.phiDays} days — residue would fail testing at the mandi.
                  </p>
                )}
              </Panel>
            )}

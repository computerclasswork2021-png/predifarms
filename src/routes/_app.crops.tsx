...
import { WHEAT_CLOSEUP } from "@/lib/photography";
  ...
  head: () => ({
    meta: [
      { title: "Crop Planner — PREDI-FARM X" },
      {
        name: "description",
        content:
          "Crop suggestions only for land that is actually free — standing crops are never told to switch.",
      },
    ...
    .map((crop: CropDef) => {
      ...
      return {
        ...
        reasons: [
          ...
          waterFit < 1
            ? "Needs more water than this block currently holds"
            : "Water demand matches this block",
          phFit < 1
            ? `pH ${b.soil.ph} is high for this crop`
            : `Suits ${b.soilType.toLowerCase()}`,
        ],
      };
    })
    .sort((x: Ranked, y: Ranked) => y.score - x.score)
    .slice(0, 3);
}
        ...
        {free.length ? (
          free.map((b) => (
            <Panel key={b.id} className="p-5">
              <SectionHeading
                icon={Sprout}
                title={`${b.name} — ${b.areaHa} ha free`}
                hint={`${b.soilType} · pH ${b.soil.ph} · OC ${b.soil.organicCarbon}%`}
              />
              <div className="grid gap-3 md:grid-cols-3">
                {rankCropsForBlock(b).map((r: Ranked, i: number) => (
                  <div
                    ...
                    className={
                      i === 0
                        ? "rounded-2xl border border-primary/35 bg-primary/8 p-4"
                        : "rounded-2xl border border-border/60 bg-surface-2/40 p-4"
                    }
                  >
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {r.crop.season} · {r.crop.cycleDays} days
                    </p>
                    <p className="mt-3 font-mono text-sm font-semibold text-primary">
                      {formatINR(r.netPerHa * b.areaHa)}
                    </p>
          ...
          <EmptyState
            icon={Sprout}
            title="Every block is committed"
            body="There is no free land this season, so there is no crop decision to make. Come back after harvest."
          />
        )}

        <Panel className="p-5">
          <SectionHeading
            title="Committed land"
            hint="Locked until harvest — shown so you know why there is no advice here."
          />
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {s.stage.name} · harvest {formatDate(s.harvestDate)}
                    </p>
                  </div>

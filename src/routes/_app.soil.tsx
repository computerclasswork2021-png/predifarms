...
export const Route = createFileRoute("/_app/soil")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "NPK, pH, organic carbon and moisture for every block, each read against the crop actually growing there." },
      { property: "og:title", content: "Soil — PREDI-FARM X" },
      { property: "og:description", content: "NPK, pH, organic carbon and moisture for every block, each read against the crop actually growing there." },
    ],
  }),
  component: SoilPage,
});
              ...
              <Reveal key={b.id} delay={i * 0.04}>
                <Panel className="h-full p-5">
                  <SectionHeading
                    icon={FlaskConical}
                    title={b.name}
                    hint={state ? `${state.crop.label} · ${state.stage.name}` : "Fallow · reading is a baseline for the next crop"}
                  />
                  <ul className="space-y-3">
                    {([["Nitrogen", b.soil.n, TARGETS.n], ["Phosphorus", b.soil.p, TARGETS.p], ["Potassium", b.soil.k, TARGETS.k]] as const).map(([label, v, target]) => (
                      <li key={label}>
                        <div className="flex items-baseline justify-between text-xs">
                          <span className="font-medium">{label}</span>
                          <span className="font-mono text-muted-foreground">{v} / {target} kg/ha</span>
                        </div>
                        <Meter className="mt-1.5" value={(v / target) * 100} tone={v >= target * 0.85 ? "good" : "warn"} />
                      </li>
                    ))}
                  </ul>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Pill tone={b.soil.ph > 7.5 || b.soil.ph < 6 ? "warn" : "good"}>pH {b.soil.ph}</Pill>
                    <Pill tone={b.soil.moisture < 45 ? "warn" : "good"}>Moisture {b.soil.moisture}%</Pill>
                    <Pill tone={b.soil.organicCarbon < 0.5 ? "warn" : "good"}>OC {b.soil.organicCarbon}%</Pill>
                  </div>

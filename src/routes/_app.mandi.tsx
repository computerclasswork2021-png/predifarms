...
import { HARVEST_COMBINE } from "@/lib/photography";
  ...
  head: () => ({
    meta: [
      { title: "Mandi Prices — PREDI-FARM X" },
      {
        name: "description",
        content:
          "Local mandi rates against MSP, with a plain answer on whether to sell now or hold.",
      },
...
function Page() {
  ...
  const standing = blocks
    .map((b) => ({ b, s: cropStateFor(b.id) }))
    .filter((x) => x.s);
  return (
    <>
      <PhotoHero
        photo={HARVEST_COMBINE}
        eyebrow="Market"
          ...
          {standing.map(({ b, s }) => {
            ...
            return (
              <Panel key={b.id} className="p-5">
                <SectionHeading
                  icon={TrendingUp}
                  title={`${crop.label} — ${b.name}`}
                  hint={`Harvest in ${s!.daysToHarvest} days`}
                />
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <StatTile
                    ...
                    value={`₹${msp}`}
                    sub="per quintal"
                    tone={aboveMsp ? "good" : "warn"}
                  />
                  <StatTile
                    label="Your lot"
                    value={econ ? `${econ.quintals} q` : "—"}
                    sub={econ ? formatINR(econ.grossRevenue) : ""}
                  />
                </div>
